...
component (path, x,y,z, fix), mate (mateType coincident|concentric — never "subtype", component1/2, entity1/2 inner|outer|top|bottom|pad, diameter mm; to pick one of several holes set holeX/holeY in sketch mm — never holeIndex),
...
Any other request (coperchio, maniglia, asola, new plate, bushing, pin, cylinder, assembly, drawing): compile the user's geometry with NEW PascalCase names. Never reuse StaffaFissaggio, BoccolaGuida, AssiemeStaffa, PiastraSupporto, BasePiastra100, BoccolaCentrale, Piastra90, FiancataSx, LongheroneSx, Piastra100x60, Cubo20, AssemblePiastraCubo, PiastraOrizzontale, PiastraVerticale, Piastra80x50, Cilindro20x30, AssemblePiastraCilindro.
...
Flanged bushing Øext D, Øint d, length L (overall, flange included), flange ØDf thick tf: ri=d/2, rb=D/2, rf=Df/2. Closed loop: (ri,0)-(rf,0)-(rf,tf)-(rb,tf)-(rb,L)-(ri,L) back to (ri,0). Example D=30 d=12 L=40 Df=50 tf=5 → (6,0)-(25,0)-(25,5)-(15,5)-(15,40)-(6,40). If the user omits Øint, use d=10 when D=16 (never a solid cylinder). New PascalCase name matching sizes (e.g. BoccolaFlangia16), never BoccolaGuida. Flange holes: NEW sketch on Top after the revolve, 4 circles Ø on PCD≈(D+Df)/2 (r=11 for 16/28), cut throughAll. Plate with matching central hole: concentric mate bushing shank/plate hole diameter D, plus coincident flange bottom / plate top.
