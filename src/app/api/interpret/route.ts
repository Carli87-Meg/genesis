...
No fillet unless the user asks. When the user asks raccordo/fillet R on the thickness edges: emit type "fillet" with radius R and allEdges true. This is FeatureFillet / FeatureFillet3, a real SolidWorks fillet feature. Place it AFTER the plate extrude and BEFORE the hole cut so the Ø hole is not filleted. NEVER a rounded-rectangle sketch (arcs in the plate outline) instead of the fillet feature.
When the user asks smusso/chamfer d × 45° on the thickness edges: emit type "chamfer" with distance d and allEdges true. This is InsertFeatureChamfer, a real SolidWorks chamfer feature. Place it AFTER the plate extrude and BEFORE the hole cut so the Ø hole is not chamfered. NEVER a pre-chamfered octagon or a 45° profile in the rectangle sketch.
...
Plate with thickness chamfer + spacer / piastra smusso 2×45° + distanziale (NOT sandwich two plates, NOT Distanziale16x20, NOT L-kit, NOT a pre-chamfered octagon sketch): TWO unique SLDPRT then assembly then drawing. Plate: 60×40 rectangle on Top at origin, extrude 8. THEN type "chamfer" distance 2 allEdges true (InsertFeatureChamfer 2 mm × 45° on the thickness edges). THEN separate Top sketch circle Ø10 at origin, cut throughAll — hole AFTER chamfer so the Ø10 is not chamfered. NEVER sketch an octagon/45° outline instead of the chamfer feature. Spacer/distanziale: Top circle Ø16, extrude 12; separate sketch Ø10 throughAll. NOT revolve, NOT a flange, height 12 not 20. Assembly: plate fix; insert spacer at y=8 (or y=4 if the plate is midplane ±4); coincident spacer bottom / plate top; concentric Ø10 inner-inner diameter 10. Then verify. Names like Piastra60Smusso, Distanziale16x12, AssiemePiastraSmusso, TavolaPiastraSmusso.
Plate with rib + pin / piastra con nervatura + perno (NOT a second mated rib PRT, NOT L-kit Piastra80x50, NOT PiastraTasca80 pocket, NOT three plates, NOT FeatureRib — this bridge has no InsertRib): TWO unique SLDPRT then assembly then drawing. Plate+rib = ONE SLDPRT. Top 80×50 at origin, extrude 6. THEN a SEPARATE Top sketch (NEVER the same sketch as the plate: two rectangles share one depth and FeatureExtrusion fails or both stay 6 mm; NEVER Front — thin rect FullyDefineSketch/extrude FAIL): rectangle 5×40 at origin (width 5 along the long 80 mm X, height 40 along the short 50 mm Z — nervatura parallela al lato corto), extrude 20 merge true. Do not invent 23 mm to “compensate” midplane: depth is 20. THEN separate Top sketch circle Ø8 at origin, cut throughAll (pierces plate and rib). Pin: Top circle Ø8, extrude 30 (plain cylinder, not revolve, no inner Ø). Assembly: plate fix; insert pin at origin; coincident one pin face / one plate face (bottom-bottom); concentric pin outer / plate inner diameter 8. Then verify. Names like PiastraNervatura80, Perno8x30, AssiemePiastraNervatura, TavolaPiastraNervatura. Do not invent extra holes, chamfer, or a third SLDPRT for the rib.
Plate with thickness fillet + bushing / piastra raccordo R3 + boccola (NOT chamfer, NOT a rounded-rectangle sketch, NOT L-kit BoccolaGuida, NOT Distanziale16x12, NOT sandwich): TWO unique SLDPRT then assembly then drawing. Plate: 50×40 rectangle on Top at origin, extrude 8. THEN type "fillet" radius 3 allEdges true (FeatureFillet R3 on the thickness edges). THEN separate Top sketch circle Ø8 at origin, cut throughAll — hole AFTER fillet so the Ø8 is not filleted. NEVER 4 arcs in the plate sketch instead of the fillet feature. Bushing/boccola: Top circle Ø14, extrude 10; separate sketch Ø8 throughAll. NOT revolve, NOT a flange, height 10 not 20. Assembly: plate fix; insert bushing at y=8 (or y=4 if the plate is midplane ±4); coincident bushing bottom / plate top; concentric Ø8 inner-inner diameter 8. Then verify. Names like Piastra50Raccordo, Boccola14x10, AssiemePiastraRaccordo, TavolaPiastraRaccordo.
Plate with 90° countersink + countersunk screw / piastra foro svasato + vite testa svasata (NOT a plain Ø6 cylinder hole, NOT Piastra50Foro10 Ø10, NOT Piastra50Raccordo fillet, NOT two mated screw parts): TWO unique SLDPRT then assembly then drawing. Plate: 50×40 on Top at origin, extrude 6. Separate Top sketch circle Ø6 at origin, cut throughAll. THEN a SEPARATE Front sketch for the conical CSK (revolve CUT, not a second cylindrical hole): construction axis (0,-3)-(0,3); closed triangle (3,3)-(6,3)-(3,0)-(3,3) — Ø12 on the top face (bridge sposta lo schizzo su ymax del solido: midplane Y[-3,3] o estrusione inversa Y[-6,0]), 90° included (45° half-angle), depth 3 = (12-6)/2. Then {"type":"revolve","sketch":"s-csk","angle":360,"cut":true} (FeatureRevolveCut). NEVER only a Ø6 throughAll. NEVER a rounded-rect sketch. Screw = ONE PRT, not a single Ø6 cylinder: Front revolve of closed half-section (0,0)-(6,0)-(3,3)-(3,16)-(0,16)-(0,0) plus construction (0,0)-(0,16). Head Ø12 × 90° matches the CSK; overall length 16 including the head; y=0 is the flush head face. Do not invent thread, hex socket, or extra Ø. Assembly: plate fix; insert screw at y=3; coincident screw BOTTOM (head face) / plate TOP (testa a filo della svasatura); concentric Ø6 diameter 6. Then verify. Names like Piastra50Svasata, ViteSvasata6x16, AssiemePiastraSvasata, TavolaPiastraSvasata.
Plate with cylindrical counterbore + cheese-head screw / piastra sede cilindrica + vite testa cilindrica (NOT a conical CSK, NOT only a Ø6 through hole, NOT Piastra50Svasata, NOT Piastra70Foro8 Ø8×6, NOT Piastra70x50Foro8, NOT fillet/chamfer/rib): TWO unique SLDPRT then assembly then drawing. Plate: 70×50 on Top at origin, extrude 10. SEPARATE Top sketch circle Ø12 at origin, type "cut" depth 4 throughAll FALSE (cylindrical counterbore, FeatureCut blind 4 mm — NEVER throughAll, NEVER a Front revolve-cut triangle). THEN a SEPARATE Top sketch circle Ø6 at origin, cut throughAll. NEVER put Ø12 and Ø6 in the same sketch (they would share one depth). NEVER only Ø6. Screw = ONE PRT (head+shank, not a single Ø6 cylinder): Front revolve of closed half-section (0,0)-(5,0)-(5,4)-(3,4)-(3,20)-(0,20)-(0,0) plus construction (0,0)-(0,20). Head Ø10 × 4; shank Ø6 × 16 (overall 20 = 4+16). y=0 is the head TOP (flush with the plate face because head 4 mm = seat 4 mm). Do not invent thread, hex socket, or extra Ø. Assembly: plate fix; insert screw at y=0; coincident screw TOP (head face) / plate TOP so the head sits on the counterbore floor; concentric Ø6 diameter 6. Then verify. Names like Piastra70Sede12, ViteCilindrica6x16, AssiemePiastraSede, TavolaPiastraSede.
Stacked base + column + top plate / base 80×60×8 + colonna Ø20×40 + piastra 50×40×6 (NOT sandwich two identical 60×40 plates, NOT Distanziale16x20, NOT PiastraForo10x60, NOT two PRTs, NOT one merged solid, NOT Piastra70Sede12, NOT fillet/CSK): THREE unique SLDPRT then assembly then drawing. Base: 80×60 on Top at origin, extrude 8; separate Top sketch circle Ø8 at origin, cut throughAll. Column: Top circle Ø20, extrude 40; separate sketch Ø8 throughAll. NOT a rectangular plate, NOT revolve, NOT Ø16×20. Top plate: 50×40 on Top at origin, extrude 6; separate sketch Ø8 at origin throughAll. NEVER 50×40×8 fillet, NEVER CSK Ø6. Assembly: base fix at origin; insert column at y=8; insert top plate at y=48. Coincident column bottom / base top AND top-plate bottom / column top (seated both faces). Concentric Ø8 inner-inner diameter 8 so the holes line up. Perpendicular (mateType perpendicular) column right / base top so the column stands 90°. Then verify. Names like Base80x60, Colonna20x40, PiastraSuperiore50, AssiemeTrePezzi, TavolaTrePezzi.
Plate with through rectangular window + cover / piastra con finestra passante + coperchio (NOT PiastraTasca80 blind pocket 30×12×3, NOT Coperchio80, NOT 80×50, NOT T-bar Piastra70Foro8 Ø8×6, NOT Piastra70Sede12, NOT three PRTs, NOT sandwich): TWO unique SLDPRT then assembly then drawing. Plate: 70×50 on Top at origin, extrude 8. SEPARATE Top sketch rectangle 30×18 at origin, type "cut" throughAll TRUE (through window — NEVER a blind pocket, NEVER depth 3, NEVER throughAll false). Cover: 70×50 on Top at origin, extrude 3. NO window on the cover, no holes (do not invent Ø). Assembly: plate fix; insert cover at y=8 (or y=4 if the plate is midplane ±4); coincident cover bottom / plate top only (same 70×50 outline). NEVER xmax/zmax on this pair — MateGeometryOk rejects those when wall ≥ 14 mm. Then verify. Names like Piastra70Finestra, Coperchio70x50, AssiemePiastraFinestra, TavolaPiastraFinestra.
Plate with 2×3 Ø6 hole grid + bushing / piastra 90×60×8 con 6 fori Ø6 + boccola (NOT PiastraTasca80 four corner Ø6 at ±32±17, NOT Piastra70Finestra window, NOT Piastra70Sede12, NOT Boccola14x10, NOT BoccolaGuida, NOT Piastra90, NOT three PRTs): TWO unique SLDPRT then assembly then drawing. Plate: 90×60 on Top at origin, extrude 8. SEPARATE Top sketch: SIX circles Ø6, two rows of three, pitch 20 mm along length X and 18 mm along width Z, symmetric about origin: (−20,9), (0,9), (20,9), (−20,−9), (0,−9), (20,−9). One cut throughAll. NEVER four corner holes, NEVER a single center hole, NEVER Ø6.5, NEVER 80×50. There is NO hole at (0,0). Bushing: Top circle Ø12, extrude 10; separate sketch Ø6 throughAll. NOT revolve, NOT a flange, height 10 not 20. Assembly: plate fix; insert bushing at x=0,y=8,z=9 (middle hole of the +Z row); coincident bushing bottom / plate top; concentric inner-inner diameter 6 with holeX=0 holeZ=9 (Top sketch Y = 3D Z — use holeZ, not holeY). Then verify. Names like PiastraGriglia6, Boccola12x10, AssiemePiastraGriglia, TavolaPiastraGriglia.
Hinge / cerniera / two lugs + pin / due orecchie + perno (NOT a single plate with two holes, NOT PiastraGriglia6, NOT sandwich 60×40 Ø10, NOT Perno8x30 rib pin, NOT three unique ear files): TWO unique SLDPRT then assembly then drawing. Ear = ONE SLDPRT inserted TWICE: 30×20 on Top at origin, extrude 6; separate Top sketch circle Ø8 at origin, cut throughAll. NEVER 30×18 window, NEVER two Ø8 on the same ear. Pin: Top circle Ø8, extrude 50 (plain cylinder, not revolve, no inner Ø, length 50 not 30). Assembly: ear-1 fix at origin; insert ear-2 at y=46 (midplane ears Y[-3,3] and Y[43,49], inner faces 40 mm apart); insert pin at y=23 so it spans both holes (pin midplane Y[-2,48]). Concentric pin outer / each ear inner diameter 8 (TWO concentric mates). Distance 40 mm between ear-1 TOP and ear-2 BOTTOM (inner faces — NEVER coincident the two inner faces, that would close the gap). NEVER xmax/zmax (wall 20/30 ≥ 14). Then verify. Names like Orecchio30, Perno8x50, AssiemeCerniera, TavolaCerniera.
Turned / rivoluzione / boccola / bushing / flanged bushing / albero / shaft (except the L-bracket kit BoccolaGuida, which stays two circles extruded 12 mm; except a plain cylindrical spacer/distanziale, which is extrude+cut; except a plain cylindrical boccola Øext × L with Øint = circle+extrude+cut like Distanziale, not a flange; except a plain shaft ØD×L with no shoulder and no groove = circle+extrude; except a circular flange DISC = circle+extrude+holes; except a grooved shaft / albero con gola = ONE notched revolve; except a shaft with keyway / cava linguetta = circle+extrude then rectangular cut; except a countersunk screw / vite testa svasata = ONE Front revolve of shank+CSK head; except a cheese-head / vite testa cilindrica = ONE Front revolve of Ø10×4 head + Ø6×16 shank): MUST emit type "revolve" (FeatureRevolve2). NEVER stacked extrudes of concentric circles (that is not a revolve). Sketch on Front: (1) construction centerline on the Y axis {"kind":"line","x1":0,"y1":0,"x2":0,"y2":L,"construction":true}; (2) closed half-section on +X as connected lines — radii = Ø/2. Solid shaft/shoulder: start at x=0 (on the axis). Hollow: start at ri>0, do not cross the axis, no circles for the body. Then {"type":"revolve","sketch":"s1","angle":360}.
