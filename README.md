...
dotnet bridge\bin\Release\net8.0-windows\swiax1ub.dll
```

Su questo PC AppLocker può bloccare l’`.exe` apphost e Smart App Control può bloccare un hash già visto di un DLL. L’assembly attuale è `swiax1ub.dll`; `dotnet …\swiax1ub.dll` è il modo supportato. `.\start-local.ps1` fa lo stesso.
