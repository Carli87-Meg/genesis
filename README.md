...
Su questo PC AppLocker può bloccare l’`.exe` apphost e Smart App Control può bloccare un hash già visto di un DLL. L’assembly attuale è `swiahost.dll`; `dotnet …\swiahost.dll` è il modo supportato. `.\start-local.ps1` fa lo stesso.
