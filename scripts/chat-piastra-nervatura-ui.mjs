 ...
 * Prompt italiano: piastra 80×50×6 con nervatura 40×5×20 nello stesso PRT + perno Ø8×30.
 */
...
const PROMPT =
  "Piastra 80 × 50 × 6 con nervatura 40 × 5 alta 20 al centro, parallela al lato corto. Foro Ø8 al centro della piastra. Perno Ø8 lunghezza 30. Perno nel foro, faccia coincidente. Quote vere su ogni schizzo. Tavola A3 Cartiglio_CM."
const OUT = path.join("sw-out", "chat-piastra-nervatura")
...
try {
  ...
  if (partDocs.length !== 2) {
    throw new Error(`piastra+perno = 2 PRT (nervatura nello stesso PRT), parti=${partDocs.join(",")}`)
  }
  ...
  if (!extrudes.includes(6) || !extrudes.includes(20) || !extrudes.includes(30)) {
    throw new Error(`estrusioni attese 6+20 (piastra/nervatura) e 30 (perno), got ${extrudes.join(",")}`)
  }

  const parts = job.filter((d) => d.document?.type === "part")
  const plate =
    parts.find((d) => {
      const depths = (d.operations || []).filter((op) => op.type === "extrude").map((op) => Number(op.depth))
      return depths.includes(6) && depths.includes(20)
    }) || parts.find((d) => /piastra|nervatur|plate|rib/i.test(d.document?.name || ""))
  if (!plate) throw new Error("manca piastra con estrusioni 6 e 20")
  ...
  const plate80 = plateRects.some(
    (r) =>
      (Math.abs(Number(r.width) - 80) < 0.2 && Math.abs(Number(r.height) - 50) < 0.2) ||
      (Math.abs(Number(r.width) - 50) < 0.2 && Math.abs(Number(r.height) - 80) < 0.2),
  )
  if (!plate80) throw new Error("manca rettangolo piastra 80×50")
  const rib40 = plateRects.some(
    (r) =>
      (Math.abs(Number(r.width) - 5) < 0.2 && Math.abs(Number(r.height) - 40) < 0.2) ||
      (Math.abs(Number(r.width) - 40) < 0.2 && Math.abs(Number(r.height) - 5) < 0.2),
  )
  if (!rib40) throw new Error("manca rettangolo nervatura 5×40 nello stesso PRT")
  const plateSketches = (plate.operations || []).filter((op) => op.type === "sketch")
  const plateAndRibSameSketch = plateSketches.some((op) => {
    const rects = (op.contours || []).filter((c) => c.kind === "rectangle" || c.type === "rectangle")
    return rects.length >= 2
  })
  if (plateAndRibSameSketch) throw new Error("piastra e nervatura nello stesso schizzo")
  ...
  const through = (plate.operations || []).some((op) => op.type === "cut" && op.throughAll === true)
  if (!through) throw new Error("foro Ø8 deve essere throughAll dopo la nervatura")
  if ((plate.operations || []).some((op) => op.type === "chamfer")) {
    throw new Error("nessuno smusso nel prompt")
  }

  const pin = parts.find((d) => d !== plate)
  if (!pin) throw new Error("manca perno")
  if ((pin.operations || []).some((op) => op.type === "revolve")) {
    throw new Error("perno deve essere estrusione, non revolve")
  }
  const pin30 = (pin.operations || []).some((op) => op.type === "extrude" && Number(op.depth) === 30)
  if (!pin30) throw new Error("manca estrusione 30 del perno")
  const pinCircles = (pin.operations || [])
    .filter((op) => op.type === "sketch")
    .flatMap((op) => (op.contours || []).filter((c) => c.kind === "circle" || c.type === "circle"))
  if (!pinCircles.some((c) => Number(c.diameter) === 8)) throw new Error("manca Ø8 perno")
  if (pinCircles.some((c) => Number(c.diameter) !== 8)) {
    throw new Error(`perno: niente Øinterno inventato, got ${pinCircles.map((c) => c.diameter).join(",")}`)
  }
