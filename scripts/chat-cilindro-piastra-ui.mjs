 ...
 * Prompt italiano nuovo: cilindro su piastra, niente kit/demo.
 */
...
const PROMPT =
  "cilindro Ø20 h30 su piastra 80×50×6, coincidente sulla faccia superiore, 4 fori Ø5 sulla piastra, quote sugli schizzi, tavola A3"
const OUT = path.join("sw-out", "chat-cilindro-piastra")
...
try {
  ...
  console.log(
    ...
    (interpretBody.job || []).map((d) => d.document?.name),
  )
  ...
  const banned =
    /StaffaFissaggio|BoccolaGuida|AssiemeStaffa|Piastra90|PiastraBase|PiastraSupporto|BasePiastra100|Piastra100x60|Cubo20|AssemblePiastraCubo|TavolaPiastraCubo|PiastraOrizzontale|PiastraVerticale|demo/i
