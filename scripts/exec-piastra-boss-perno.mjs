 ...
 * Coincidente pad sceglieva la piastra, non la corona del boss: payload sanificato top/top Ø8.
 */
...
const job = src.job
const plate = job.find((d) => d.document?.type === "part" && /piastra|boss/i.test(d.document.name))
...
for (let i = 0; i < docs.length; i++) {
  ...
  const faces = json.steps?.filter((s) => /selectFace/i.test(s.op || "")) || []
  for (const f of faces) console.log("face", f.ok, f.detail)
  if (!json.ok) {
