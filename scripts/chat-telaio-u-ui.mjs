 ...
 * Prompt italiano: telaio a U, due montanti identici = un PRT ×2.
 */
...
const PROMPT =
  "Telaio a U: piastra base 100 × 40 × 8, due montanti 40 × 40 × 8 sui lati corti, coincidenti sulla base e perpendicolari, due fori Ø8 sulla base a 15 mm dalle estremità. Quote vere su ogni schizzo. Tavola A3 Cartiglio_CM."
const OUT = path.join("sw-out", "chat-telaio-u")
...
try {
  ...
  const banned =
    /StaffaFissaggio|BoccolaGuida|AssiemeStaffa|Piastra90|PiastraBase$|PiastraSupporto|BasePiastra100|Piastra100x60|Cubo20|Piastra80x50|Cilindro20x30|BoccolaFlangia16|PiastraForo16|AlberoSpallamento10|PiastraForo10|Assemie|demo/i
  ...
  if (partDocs.length !== 2) {
    throw new Error(`due montanti identici = un PRT ×2, parti=${partDocs.join(",")}`)
  }
  ...
  if (!mateTypes.includes("coincident") || !mateTypes.includes("perpendicular")) {
    throw new Error(`servono mate coincident+perpendicular, got ${mateTypes.join(",")}`)
  }
