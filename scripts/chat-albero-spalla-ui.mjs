 ...
 * Prompt inglese nuovo: albero con spallamento (revolve) + piastra, niente kit/demo.
 */
...
const PROMPT =
  "shaft Ø10 L80 with a Ø16 x 6 shoulder on one end, plate 50×40×6 with a Ø10 hole, concentric in the hole, coincident against the shoulder, true dimensions on every feature sketch, A3 drawing"
const OUT = path.join("sw-out", "chat-albero-spalla")
...
try {
  ...
  const banned =
    /StaffaFissaggio|BoccolaGuida|AssiemeStaffa|Piastra90|PiastraBase|PiastraSupporto|BasePiastra100|Piastra100x60|Cubo20|Piastra80x50|Cilindro20x30|BoccolaFlangia16|PiastraForo16|Assemie|demo/i
  ...
  if (!types.includes("revolve")) {
    throw new Error(`albero senza revolve: ${types.join(",")}`)
  }
