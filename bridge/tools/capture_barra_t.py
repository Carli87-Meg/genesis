"""Cattura GUI barra a T + piastra. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-barra-t")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"barra-t-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "barra-t-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "barra-t-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "barra-t-tavola-01.jpg", "barra-t-cartiglio-01.jpg")
    gui_shot("Tavola", "barra-t-gui-no-edit-01.jpg")
