"""Cattura GUI dado esagonale + piastra. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-dado-esagono")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"dado-esagono-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "dado-esagono-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "dado-esagono-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "dado-esagono-tavola-01.jpg", "dado-esagono-cartiglio-01.jpg")
    gui_shot("Tavola", "dado-esagono-gui-no-edit-01.jpg")
