"""Cattura GUI telaio a U. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-telaio-u")
...
def main() -> None:
    for name in os.listdir(EXPORT):
        ...
        if not low.endswith(".jpg"):
            continue
        if any(k in low for k in ("baseu", "piastrabaseu", "montanteu", "telaiou", "assiemetelaiou", "tavolatelaiou")):
            save_all(f"telaio-u-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "telaio-u-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "telaio-u-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "telaio-u-tavola-01.jpg", "telaio-u-cartiglio-01.jpg")
    gui_shot("Tavola", "telaio-u-gui-no-edit-01.jpg")
