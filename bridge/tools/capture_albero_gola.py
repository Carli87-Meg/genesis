"""Cattura GUI albero con gola + piastra. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-albero-gola")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"albero-gola-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "albero-gola-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "albero-gola-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "albero-gola-tavola-01.jpg", "albero-gola-cartiglio-01.jpg")
    gui_shot("Tavola", "albero-gola-gui-no-edit-01.jpg")
