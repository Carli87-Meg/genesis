"""Cattura GUI cerniera due orecchie + perno (prompt IT). Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-cerniera-orecchie")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"cerniera-orecchie-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "cerniera-orecchie-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "cerniera-orecchie-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "cerniera-orecchie-tavola-01.jpg", "cerniera-orecchie-cartiglio-01.jpg")
    gui_shot("Tavola", "cerniera-orecchie-gui-no-edit-01.jpg")
