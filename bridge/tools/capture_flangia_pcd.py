"""Cattura GUI flangia circolare PCD + albero. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-flangia-pcd")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"flangia-pcd-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "flangia-pcd-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "flangia-pcd-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "flangia-pcd-tavola-01.jpg", "flangia-pcd-cartiglio-01.jpg")
    gui_shot("Tavola", "flangia-pcd-gui-no-edit-01.jpg")
