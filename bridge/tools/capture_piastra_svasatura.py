"""Cattura GUI piastra svasata 90° + vite testa svasata. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-piastra-svasatura")
...
def main() -> None:
    keys = (
        ...
        "piastra-svasatura",
        "vitesvasata6x16",
        "assiemepiastrasvasata",
        "tavolapiastrasvasata",
    )
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"piastra-svasatura-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "piastra-svasatura-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "piastra-svasatura-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "piastra-svasatura-tavola-01.jpg", "piastra-svasatura-cartiglio-01.jpg")
    gui_shot("Tavola", "piastra-svasatura-gui-no-edit-01.jpg")
