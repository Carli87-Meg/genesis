"""Cattura GUI piastra raccordo R3 + boccola. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-piastra-raccordo")
...
def main() -> None:
    keys = (
        ...
        "piastra-raccordo",
        "boccola14x10",
        "assiemepiastraraccordo",
        "tavolapiastraraccordo",
    )
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"piastra-raccordo-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "piastra-raccordo-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "piastra-raccordo-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "piastra-raccordo-tavola-01.jpg", "piastra-raccordo-cartiglio-01.jpg")
    gui_shot("Tavola", "piastra-raccordo-gui-no-edit-01.jpg")
