"""Cattura GUI albero con cava linguetta + piastra. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-albero-cava")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"albero-cava-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "albero-cava-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "albero-cava-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "albero-cava-tavola-01.jpg", "albero-cava-cartiglio-01.jpg")
    gui_shot("Tavola", "albero-cava-gui-no-edit-01.jpg")
