"""Cattura GUI piastra+boss cilindrico + perno. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-piastra-boss-perno")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"piastra-boss-perno-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "piastra-boss-perno-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "piastra-boss-perno-ui-eseguito-01.jpg"),
        ("ui-fail.jpg", "piastra-boss-perno-ui-proponi-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "piastra-boss-perno-tavola-01.jpg", "piastra-boss-perno-cartiglio-01.jpg")
    gui_shot("Tavola", "piastra-boss-perno-gui-no-edit-01.jpg")
