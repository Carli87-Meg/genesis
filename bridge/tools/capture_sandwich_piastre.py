"""Cattura GUI sandwich due piastre + distanziale. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-sandwich-piastre")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"sandwich-piastre-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "sandwich-piastre-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "sandwich-piastre-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "sandwich-piastre-tavola-01.jpg", "sandwich-piastre-cartiglio-01.jpg")
    gui_shot("Tavola", "sandwich-piastre-gui-no-edit-01.jpg")
