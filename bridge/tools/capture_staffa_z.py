"""Cattura GUI staffa a Z + piastra. Solo HWND, niente GetObject/CloseDoc."""
...
CHAT = os.path.join(os.path.dirname(__file__), "..", "..", "sw-out", "chat-staffa-z")
...
def main() -> None:
    ...
    for name in os.listdir(EXPORT):
        ...
        if any(k in low for k in keys):
            save_all(f"staffa-z-{name}", os.path.join(EXPORT, name))
    for ui_name, dest in (
        ("ui-proposta.jpg", "staffa-z-ui-proposta-01.jpg"),
        ("ui-eseguito.jpg", "staffa-z-ui-eseguito-01.jpg"),
    ):
        src = os.path.normpath(os.path.join(CHAT, ui_name))
        if os.path.isfile(src) and os.path.getsize(src) > 0:
            save_all(dest, src)
    gui_shot("Tavola", "staffa-z-tavola-01.jpg", "staffa-z-cartiglio-01.jpg")
    gui_shot("Tavola", "staffa-z-gui-no-edit-01.jpg")
